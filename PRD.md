...
| 2 | **销售总监** | 3 - 本部门组数据权限（phase8 拆档后从原 4『本部门及以下』改来） | 查看全盘销售漏斗、修改公海池回收规则、拥有最高分配权。 |
| 3 | **销售主管** | 3 - 本部门组数据权限 | 查看并审批下属的合同、商机；可手动划拨团队内的客户。 |
...
* **访问边界**：**系统管理员（admin）+ 销售总监** 2 个角色可见可用；销售主管 / 销售 / 财务均**无权访问**。所有写操作都走 Sa-Token 鉴权（`sys:*:edit` 权限码）+ `@SaCheckRole({"admin","sales_director"})` 双层校验。
    * **理由**：公司体量小，admin 忙不过来时销售总监需能顶岗（开账号 / 调权限 / 改组织），销售总监已有的"3-本部门组"数据范围（在总公司 dept=1 时等价于看全部）已能查看下属所有客户，顺手管人/管权限合理。
...
* **data_scope 说明**（沿用阶段一定义）：
    ...
    | 1 | 全部 | 不拼数据权限 |
    | 2 | 自定义 | 预留扩展（V1 未启用） |
    | 3 | 本部门组 | 拼接 `owner_user_id IN (我的部门 OR 同 parent_id 兄弟部门的所有用户)`（phase8 commit1 改造）|
    | 4 | ~~本部门及以下~~ | phase8 commit1 拆档；该档已废，代码 case 4 删，DB 历史值兜底为仅本人 |
